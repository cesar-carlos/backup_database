...
- [x] F2.10 Implementar fila FIFO para execucoes agendadas quando houver backup ativo. _(PR-3a + F2.16 — `ExecutionQueueService` com espelhamento Drift opcional; producao usa persistencia)_
...
- [x] F2.13 Definir e validar maquina de estados da execucao (`queued`, `running`, `completed`, `failed`, `cancelled`) no servidor. _(PR-3a commit 3 — `ExecutionStateMachine` com tabela explicita de transicoes permitidas + `enforceTransition` + `InvalidStateTransitionException`)_
...
- [~] F2.16 Persistir estado de execucao e fila para recuperacao apos reinicio do servidor. _(fila persistida: `execution_queue_items_table` + `DriftExecutionQueuePersistence` + `ExecutionQueueService.initialize()` no bootstrap — 2026-04-19; **estado de execucao em curso** / `RemoteExecutionRegistry` ainda volatil apos restart)_
...
- [x] Entregar diagnostico e suporte operacional: _(PR-3 commit final, contratos prontos; impl real via DI)_
  - [x] `getRunLogs`
  - [x] `getRunErrorDetails`
  - [x] `getArtifactMetadata`
  - [x] `cleanupStaging` remoto
