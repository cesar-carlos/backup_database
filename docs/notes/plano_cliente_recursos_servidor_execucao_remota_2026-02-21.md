...
Status: **PR-1 + PR-2 + PR-3 + Wirings DI completos (2026-04-19)** — PR-1: 6 endpoints de inspecao do servidor + envelope REST-like + testes de seguranca formal do handshake. PR-2: testDatabaseConnection + IdempotencyRegistry + startBackup nao-bloqueante (M2.2) + cancelBackup + Schedule CRUD + Database config CRUD (5 commits). PR-3: ExecutionQueueService FIFO + queueIfBusy + drain automatico + eventos de fila (backupQueued/Dequeued/Started com eventId UUID + sequence monotonico) + cancelQueuedBackup + ExecutionStateMachine explicita + endpoints de diagnostico (4 commits). **Wirings DI concretos**: RealDatabaseConnectionProber (despacha aos 3 backup services), RealDatabaseConfigStore (despacha aos 3 repositories), RealDiagnosticsProvider (best-effort v1 reusa BackupHistory/Log + filesystem de staging), QueueEventBus cabeado ao TcpSocketServer.sendToClient via setter (resolve dependencia circular). **583 testes na suite, 0 erro analyze (apenas info-level)**. **Sistema funcional fim-a-fim em producao** (resta apenas persistencia da fila — F2.16 — que requer migracao de schema). PR-4/PR-5 (transferencia resiliente, observabilidade completa) ainda nao iniciados.
