...
Status: **PR-1 essencialmente completo** — 6 endpoints de inspecao do servidor (capabilities, health, session, preflight, executionStatus, executionQueue), wire version validation, payload limits per type, runId no contrato, registry sem singleton, envelope REST-like (`success`/`statusCode`) aplicado em todas as respostas, guard explicito pre-auth, errorCode em todos handlers. **Pendente de PR-1**: F0.4 testes de seguranca formal do handshake, checks reais cabeados em preflight/health (mocks default por enquanto). **PR-2/PR-3/PR-4 ainda nao iniciados** mas estruturas-base ja preparadas (registry runId-aware, capabilities como gate, fila vazia retornavel, envelope REST-like consistente).
