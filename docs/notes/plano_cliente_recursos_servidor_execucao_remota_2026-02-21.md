...
- [~] F0.5 Definir envelope padrao de response REST-like no socket com `statusCode`, `success`, `data`, `error`. _(parcial em 2026-04-19: erros ja emitem `statusCode`; sucessos com envelope completo migram handler-a-handler em commits futuros)_
  ...
  - [ ] `success: true/false` campo top-level (ainda implicito pelo `MessageType` da resposta vs `error`).
  - [ ] Envelope `data: {...}` envolvendo payloads de sucesso (capabilities/health/session/preflight/executionStatus/executionQueue/metrics) — cada handler precisa migrar ou opt-in via wrapper.
