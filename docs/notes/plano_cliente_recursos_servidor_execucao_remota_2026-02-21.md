...
Status: **PR-1 oficialmente completo (2026-04-19)** — 6 endpoints de inspecao do servidor (capabilities, health, session, preflight, executionStatus, executionQueue), wire version validation, payload limits per type, runId no contrato, registry sem singleton, envelope REST-like (`success`/`statusCode`) aplicado em todas as respostas, guard explicito pre-auth, errorCode em todos handlers, **testes formais de seguranca do handshake** (F0.4: 28 testes em 3 arquivos cobrindo PasswordHasher properties, e2e do handshake, defesa contra enumeracao). **Pendente apenas refinamento**: checks reais cabeados em preflight/health (mocks default por enquanto, mas contratos completos). **PR-2/PR-3/PR-4 ainda nao iniciados** mas estruturas-base ja preparadas (registry runId-aware, capabilities como gate, fila vazia retornavel, envelope REST-like consistente, defesas de segurança ancoradas).
...
- [~] Definir contrato de idempotencia para comandos mutaveis:
  ...
  - [ ] integracao em handlers mutaveis (`startBackup`/`cancelBackup`/`createSchedule`/`updateSchedule`) — em PR-2 commit 3.
