...
- (2026-04-19) **Wirings DI concretos entregues — PR-1/2/3 funcionais fim-a-fim**: 4 novos arquivos cabeam todos os providers stub para producao:
  ...
  - `lib/infrastructure/socket/server/real_database_connection_prober.dart`: implementa DatabaseConnectionProber despachando por RemoteDatabaseType para os 3 backup services existentes (que ja tem `testConnection`). Mede latencia via Stopwatch. Mapeia Result -> outcome com classificacao heuristica de erros (timeout/auth/io/unknown via toString matching). Pattern matching sobre sealed DatabaseConfigRef garante exhaustividade.
  - `lib/infrastructure/socket/server/real_database_config_store.dart`: implementa DatabaseConfigStore despachando por tipo aos 3 repositories existentes. Reusa serializers acima. Classificacao "not found" vs "unknown" baseada em mensagem do exception. ArgumentError do serializer mapeia para errorCode.invalidRequest (payload malformado != bug do servidor).
  - `lib/infrastructure/socket/server/real_diagnostics_provider.dart`: implementa DiagnosticsProvider em modo **best-effort v1**: extrai scheduleId do runId (formato `<scheduleId>_<uuid>`) e consulta BackupHistory/Log existentes; staging metadata via list de arquivos em `<stagingBase>/remote/<scheduleId>/`; cleanup deleta diretorio inteiro reportando bytesFreed. Hash SHA-256 streaming (suporta arquivos de qualquer tamanho via `sha256.bind`). Em PR final substituir lookup por query direta quando coluna `runId` for adicionada.
  - `lib/core/di/infrastructure_module.dart`: registra IdempotencyRegistry + ExecutionQueueService como singletons compartilhados; ExecutionMessageHandler + ScheduleCrudMessageHandler + DatabaseConfigMessageHandler + DiagnosticsMessageHandler com dependencias reais; ExecutionQueueMessageHandler usa snapshot do queueService como QueueProvider; QueueEventBus cabeado ao TcpSocketServer.sendToClient via setter (resolve dependencia circular `ExecutionHandler ↔ EventBus ↔ Server`); TcpSocketServer recebe TODOS os 16 handlers cabeados.
  - 30 novos testes (5 serializers + 14 prober + 11 store). **Sistema operacional em producao do ponto de vista de wiring** — persistencia da fila entregue em 2026-04-19 (ver bullet seguinte).
...
- (2026-04-19) **PR-2 commit 3: `startBackup` nao-bloqueante (M2.2) + `cancelBackup` entregues**: 4 novos MessageType (startBackupRequest/Response, cancelBackupRequest/Response); `lib/infrastructure/protocol/execution_messages.dart` com factories + snapshots tipados (StartBackupResult, CancelBackupResult) + envelope REST-like (202 Accepted para aceite assincrono, 200 para sincrono ex cache idempotente, 409 para noActiveExecution). XOR contratual no cancelBackup: cliente passa runId (preferido) OU scheduleId (compat). `lib/infrastructure/socket/server/execution_message_handler.dart` com: (a) startBackup nao-bloqueante: valida (schedule existe, license OK, slot livre), reserva runId, dispara executeBackup em background (unawaited), responde IMEDIATAMENTE com runId + state=running + 202; eventos backupProgress/Complete/Failed chegam separados via stream com mesmo runId; executeSchedule legacy (bloqueante) continua existindo no ScheduleMessageHandler para compat v1; (b) cancelBackup: resolve scheduleId a partir de runId quando necessario, delega ao SchedulerService.cancelExecution, responde com state=cancelled/notFound/failed; (c) Idempotency integration: ambos handlers usam IdempotencyRegistry.runIdempotent — idempotencyKey opcional permite cliente reenviar com seguranca (mesma chave dentro do TTL retorna mesma resposta cacheada sem disparar nova execucao); falhas de validacao NAO sao cacheadas; (d) Excecao interna StartFailure para sinalizar erros sem cachea-los; (e) Background task runBackupAsync captura toda excecao para garantir que falha nao-tratada nao vire uncaught zone error. ConnectionManager.startRemoteBackup() e cancelRemoteBackup() na fronteira do cliente. 32 testes do handler + protocolo cobrindo XOR, idempotencia, falhas validacao, fail-NO-cache, async non-blocking. 5 fixtures golden. 439 testes na suite, 0 issue analyze.
...
- (2026-04-19) `getExecutionQueue` endpoint entregue (PR-3b base): novos `MessageType.executionQueueRequest/Response`; `lib/infrastructure/protocol/execution_queue_messages.dart` com factories + `QueuedExecution` (runId, scheduleId, queuedAt, queuedPosition, requestedBy opcional) + `ExecutionQueueResult` snapshot tipado com `isEmpty`/`isFull`/`availableSlots` getters. `ExecutionQueueMessageHandler` com `QueueProvider` typedef injetavel + ordenacao defensiva por `queuedPosition` asc + fail-soft. **Producao:** `QueueProvider` cabeado a `getIt<ExecutionQueueService>().snapshot()` (fila em memoria sincronizada com Drift via F2.16). Dados duraveis: tabela `execution_queue_items_table`. 3 fixtures golden (request, response_empty, response_with_items). Cobertura historica: 18 testes novos na entrega inicial. Cliente pode consultar fila real apos PR-3a/F2.16.
...
- ~~Sem envelope de resposta REST-like com `statusCode` padronizado~~ — entregue em 2026-04-19 (F0.5/F0.6 + `response_envelope.dart` + golden)
- ~~Sem tabela oficial de status code para operacoes remotas~~ — entregue em 2026-04-19 (`status_codes.dart` + mapping `ErrorCode` -> HTTP-like)
- ~~Sem `capabilitiesRequest` / `capabilitiesResponse` no protocolo~~ — entregue em 2026-04-19
- ~~Sem CRUD remoto de configuracao de banco (`createDatabaseConfig`, etc.)~~ — entregue em 2026-04-19 (PR-2 + wirings reais)
...
- ~~Sem endpoint para cancelar item enfileirado (`cancelQueuedBackup`)~~ — entregue em 2026-04-19 (PR-3a)
- ~~Sem endpoint para metadados do artefato (`getArtifactMetadata`)~~ — entregue em 2026-04-19 (diagnosticos PR-3)
- ~~Sem endpoint de diagnostico por execucao (`getRunLogs`, `getRunErrorDetails`)~~ — entregue em 2026-04-19 (PR-3)
- ~~Sem comandos de agendamento completos (`createSchedule`, `deleteSchedule`, `pauseSchedule`, `resumeSchedule`)~~ — entregue em 2026-04-19 (PR-2 commit 4); `updateSchedule` permanece no handler legado
- ~~Sem idempotencia por `idempotencyKey` em comandos mutaveis~~ — entregue em 2026-04-19 (`IdempotencyRegistry`); F2.14 (tornar chave **obrigatoria**) ainda aberto
...
- Garantir limite de 1 backup em execucao por servidor — PR-3a (mutex + fila).
- Garantir que disparo manual durante execucao ativa seja rejeitado com erro claro para o cliente — `409` + `BACKUP_ALREADY_RUNNING` / fila conforme `queueIfBusy`.
- Garantir que disparo agendado durante execucao ativa entre em fila para execucao posterior — `ExecutionQueueService` + fila Drift.
- Entregar ao cliente o arquivo final pronto para continuar o fluxo de distribuicao aos destinos finais (download via transferencia existente).
...
- Disparo manual com backup ativo retorna conflito (`409` / fila) conforme `queueIfBusy`.
- Disparo com servidor ocupado pode enfileirar e retornar `202` (fila) quando habilitado.
- Fila FIFO (memoria + Drift).
- Itens de fila com `runId`, `scheduleId`, `queuedAt`, `requestedBy` (eventos publicam detalhe).
- Eventos `backupQueued`, `backupDequeued`, `backupStarted` (com `eventId`/`sequence`).
- `getExecutionStatus` com `queuedPosition`/`state` ricos — **PR-3c** (registry + fila + historico com `runId` v31+; sem `runId` legado ainda cai em `notFound` apos fim)
- Cancelar execucao ativa e remover item enfileirado (`cancelBackup` + `cancelQueuedBackup`)

## Regras Operacionais Criticas (Obrigatorias)

- Maquina de estados (`ExecutionStateMachine` + testes) — **codigo**; *enforcement* total em todo handler ainda [~] (refactor opcional no plano 2026-04-19)
- `IdempotencyRegistry` in-memory (TTL) em comandos chave; [ ] persistencia pos-restart (F2.16 idempotency — futuro) e [ ] chave **obrigatoria** (F2.14)
...
- Eventos de fila com `eventId` + `sequence`; correlacao re-sync M8.4 ainda [~] (cenarios/testes)
...
- `StatusCodes` + `isRetryable` (base F0); matriz operacional "acao do cliente" [~] documentacao/UI
...
- Cliente baixa arquivo e continua fluxo local de envio para destinos finais.
...
- Envelope com `statusCode`/`success` para handlers de inspecao e erros; estrategia aditiva top-level (ver F0.5).
- [~] No fluxo remoto, cliente nao deve depender de pasta de download local para **autorizar** execucao no servidor. *(ainda ha validacoes cedo no fluxo — PR-4/UX)*
...
- No maximo 1 backup em execucao por vez (fila + mutex).
- Concorrencia adicional enfileirada (Drift + memoria).
- `requestId` no header binario v1; `runId`/`idempotencyKey` no payload.
- Staging remoto, metadados e cleanup **por `runId`** (pendente PR-4).

## Responsabilidade de Execucao (Servidor x Cliente)

- Somente servidor executa: teste de conexao de banco, dump, compactacao, checksum, scripts, limpeza e preparacao de artefato final.
...
- Auth e sessao: *(trio do handshake completo em 2026-04-19)*
  - `authRequest`, `authResponse`
  - `capabilitiesRequest`, `capabilitiesResponse`
  - `healthRequest`, `healthResponse`
  - `sessionRequest`, `sessionResponse`
- Configuracao de banco: *(PR-2 + database_config handlers — 2026-04-19)*
  - `createDatabaseConfig`, `updateDatabaseConfig`, `deleteDatabaseConfig` (mutacoes com idempotencia)
  - `listDatabaseConfigs` (sem `getDatabaseConfigById` dedicado — uso via list + id; evoluir se necessario)
  - `testDatabaseConnection`
- [~] Preflight: *(infraestrutura entregue em 2026-04-19)*
  - `preflightRequest`, `preflightResponse` (`validateServerBackupPrerequisites`)
...
- Execucao remota sob comando do cliente:
  - `startBackup` / `cancelBackup` (PR-2) + `executeSchedule`/`cancelSchedule` (legado)
  - `getExecutionStatus` *(PR-3c: queued + terminais via fila/historico com `runId` v31+)*
  - `getExecutionQueue` + fila Drift
  - `cancelQueuedBackup`
- Execucao e progresso:
  - `backupProgress`, `backupComplete`, `backupFailed` (com `runId` opcional desde M2.3)
  - `backupQueued`, `backupDequeued`, `backupStarted` (PR-3a)
- Artefato e diagnostico:
  - `listFiles`, `fileTransferStart`, `fileChunk`, transferencia, etc.
  - `getRunLogs`, `getRunErrorDetails`, `getArtifactMetadata`, `cleanupStaging` remoto (PR-3)
- Metricas: *(enriquecida em 2026-04-19)*
  - `metricsRequest`, `metricsResponse` (com `serverTimeUtc`, `activeRunCount`, `activeRunId`, `stagingUsageBytes`)
- Erros: *(F0)*
  - `error` com `errorCode`, `errorMessage`, `statusCode` onde aplicavel
  - `createScheduleErrorMessage` com `errorCode` + `statusCode` automatico
  - `ErrorCode` estendido (`unsupportedProtocolVersion`, `payloadTooLarge`, `backupAlreadyRunning`, etc.)
...
- Autenticacao no connect (handshake):
...
- Listar arquivos disponiveis no staging remoto:
  - mensagens: `listFiles` -> `fileList`
  - cliente: `ConnectionManager.listAvailableFiles()`
- Baixar arquivo remoto com resume:
...
- Consultar metricas do servidor:
...
- Negociar features com o servidor (capabilities):
...
- Confirmar identidade da sessao:
...
- Validar prerequisites para backup remoto (preflight):
...
- Consultar status de execucao por `runId`:
...
- Consultar fila de execucoes pendentes:
...
- `getServerHealth` / `getSession` / `capabilities` / preflight (infra) / CRUD DB / `testDatabaseConnection` / `cancelQueued` / eventos de fila / diagnostico / `statusCode` em envelope F0 — **entregues 2026-04-19** (ver avancos).
...
- F0.1 Bloquear processamento de mensagens nao-auth quando conexao ainda nao autenticada. *(entregue em 2026-04-19)*
  - `client_handler.dart` rejeita explicitamente mensagens operacionais (nao-auth) chegadas com `isAuthenticated == false`, respondendo com `ErrorCode.notAuthenticated` (401) sem desconectar — cliente ainda pode enviar `authRequest` valido na sequencia.
  - Allowlist pre-auth: `authRequest`, `heartbeat`, `disconnect`, `error`. Demais tipos -> rejeitados.
  - Pause/resume do `_socketSubscription` durante validacao async ja existente continua atuando como primeira camada (fluxo normal); guard agora cobre defesa em profundidade contra peer hostil.
  - Teste e2e em `client_handler_test.dart` validando que cliente recebe `notAuthenticated`/401 sem ser desconectado.
- F0.2 Padronizar erro remoto com `errorCode` + `errorMessage` para todos handlers. *(entregue em 2026-04-19)*
  - `createScheduleErrorMessage` agora aceita `errorCode` opcional + emite `statusCode` automatico via `StatusCodes.forErrorCode` (mantem backward-compat — sem errorCode emite apenas `statusCode = 500` fail-safe).
  - `ScheduleMessageHandler._sendError` propaga `ErrorCode` apropriado em cada cenario:
  ...
  - Novos `ErrorCode` adicionados: `backupAlreadyRunning`, `scheduleNotFound`, `noActiveExecution`, `notAuthenticated` — todos com mapeamento explicito em `StatusCodes`.
  - 7 testes novos validando que cada cenario emite `errorCode` + `statusCode` corretos.
  - Nova fixture golden `schedule_error_backup_already_running.golden.json` documenta envelope completo.
  - `metrics_message_handler.dart`: ainda nao revisado para padronizacao (mas nao usa `createScheduleErrorMessage`).
- F0.3 Adicionar endpoint de capacidades (`capabilities`) e versao de API remota. *(entregue em 2026-04-19)*
  - `MessageType.capabilitiesRequest`, `capabilitiesResponse` adicionados.
  - `CapabilitiesMessageHandler` criado e roteado em `tcp_socket_server.dart`.
  - `ConnectionManager.getServerCapabilities()` + `refreshServerCapabilities()` + auto-refresh em `connect()` + 4 getters convenientes.
  - `protocol_versions.dart` define `kCurrentWireVersion` e `kCurrentProtocolVersion` (ADR-003).
  - `BinaryProtocol` valida wire version e lanca `UnsupportedProtocolVersionException`; `client_handler` responde com `errorCode = unsupportedProtocolVersion` + disconnect apos flush.
- F0.4 Criar testes de seguranca do handshake e rejeicao pre-auth. *(entregue em 2026-04-19)*
  - Bug critico de teste corrigido: `server_authentication_test.dart` mockava `LicenseFeatures.remoteControl` enquanto o codigo real consulta `LicenseFeatures.serverConnection` — testes passavam por COINCIDENCIA via `MissingStubError → catch generico → licenseDenied` (falso positivo). Stub agora bate com a feature exata.
  - Defesa F0.4 em `ServerAuthentication.validateAuthRequest`: cast inseguro `as String?` substituido por `is String` pattern-check — payload com tipos errados (int em vez de string) agora retorna `invalidRequest` em vez de jogar `TypeError` nao-tratado.
  - 6 novos testes em `server_authentication_test.dart` (13 total): feature exata invocada, license throws sync, passwordHash vazio, payload com tipo errado, message type errado, serverId com control chars, hash length mismatch.
  - Novo `handshake_security_test.dart` com 15 testes formais agrupados em 3 dimensoes:
  ...
  - Total `server_authentication_test.dart` + `handshake_security_test.dart` + `client_handler_test.dart` cobre 19+13+15 cenarios formais de seguranca do handshake.
- F0.5 Definir envelope padrao de response REST-like no socket com `statusCode`, `success`, `data`, `error`. *(entregue em 2026-04-19, estrategia aditiva top-level)*
  - `statusCode` automatico no `createErrorMessage` derivado do `errorCode` via `StatusCodes.forErrorCode`.
  - `getStatusCodeFromMessage` helper para cliente ler quando presente (backward-compat com servidor v1).
  - `success: true/false` campo top-level via `wrapSuccessResponse` helper. Aplicado a TODOS os 7 handlers de inspecao (capabilities/health/session/preflight/executionStatus/executionQueue/metrics).
  - `getSuccessFromMessage` helper para leitura defensiva (retorna `null` em payload v1; cliente usa `?? !isErrorMessage(msg)` como fallback conservador).
  - Estrategia ADITIVA top-level (sem `data: {...}` envolvendo o payload original): permite migracao 100% sem quebrar peers ja em campo. Envelope `data` reservado para `v2`/`v3` quando houver bump real do `protocolVersion` (ADR-003).
  - Invariante de teste valida que TODOS os 7 response factories aplicam o envelope (qualquer novo handler que esquecer falha o build).
  - Todas as 11 fixtures golden de response atualizadas refletindo o novo envelope.
- F0.6 Definir tabela de mapeamento `ErrorCode -> statusCode`. *(entregue em 2026-04-19)*
  - `lib/infrastructure/protocol/status_codes.dart` com 12 codigos HTTP-like (200/202/400/401/403/404/409/410/422/429/500/503) + mapa completo cobrindo todos os 16 `ErrorCode` existentes.
  - Helpers de classificacao: `isSuccess`/`isClientError`/`isServerError`/`isRetryable` (este ultimo cobre 5xx + 429 — base para retry/backoff automatico do cliente).
  - Teste invariante: novo `ErrorCode` sem entrada explicita falha o build (forca decisao de mapping em code review).

DoD Fase 0:

- Nenhuma mensagem operacional e aceita antes de auth bem-sucedida. *(F0.1)*
- [~] Cliente recebe `statusCode` + `errorCode` + `errorMessage` nos fluxos principais; auditar handlers legados se necessario
- Cliente consegue descobrir capacidades/versao do servidor antes de operar. *(entregue em 2026-04-19 — `ServerCapabilities` snapshot com `protocolVersion`/`wireVersion`/5 flags `supports`*/`chunkSize`/`compression`/`serverTimeUtc`)*
...
- [~] F1.3 Adicionar API remota de execucao sob comando do cliente:
  - `executeBackup` *(via `executeSchedule`)*
  - `cancelBackup` *(via `cancelSchedule`)*
  - `getExecutionStatus` - polling de status *(PR-2 + **PR-3c 2026-04-23**: fila + historico; ver status topo)*
  - [~] `getExecutionQueue` - consulta de fila ativa *(endpoint entregue em 2026-04-19; provider sempre vazio em PR-1; PR-3b vai popular)*
  - [~] reutilizar `SchedulerService.executeNow/cancelExecution` e estado atual exposto em `metricsResponse` *(metricsResponse enriquecido com `activeRunId`/`activeRunCount`)*
...
- [~] F1.8 Adicionar API de preflight para validar prerequisitos de execucao no servidor, reaproveitando use cases/servicos ja existentes: *(infraestrutura entregue em 2026-04-19; checks reais ficam para PR-1)*
  - `validateServerBackupPrerequisites` endpoint implementado: `lib/infrastructure/protocol/preflight_messages.dart` + `PreflightMessageHandler` + `ConnectionManager.validateServerBackupPrerequisites()` + 3 fixtures golden.
  - Tipos: `PreflightStatus` (passed/passedWithWarnings/blocked) + `PreflightSeverity` (blocking/warning/info) + `PreflightCheckResult` com `details` estruturados + `PreflightResult` snapshot tipado com `blockingFailures` / `warnings` / `isOk`.
  - Politica de agregacao: blocking falhou -> `blocked`; warning falhou -> `passedWithWarnings`; info falhou nao escala; checks que lancam excecao tratados como blocking failure (fail-closed).
  - Defesa fail-closed em parsing: status invalido vira `blocked`; severity invalida vira `info` (nao escala arbitrariamente).
  - Limites de payload em `PayloadLimits` (1KB/64KB).
...
- F1.9 Completar comandos remotos de agendamento faltantes sem alterar regra de negocio do scheduler:
  ...
  - `pauseSchedule`
  - `resumeSchedule`
- [~] F1.10 Adicionar API de saude minima do servidor: *(infraestrutura entregue em 2026-04-19; checks de subsistemas adicionais ficam para PR-1)*
  - `getServerHealth` endpoint implementado: `lib/infrastructure/protocol/health_messages.dart` + `HealthMessageHandler` + `ConnectionManager.getServerHealth()` + 2 fixtures golden (`health_response_ok`, `health_response_degraded`).
  - Snapshot tipado `ServerHealth` no cliente com `isOk`/`isUnhealthy` getters; defesa fail-closed (status invalido vira `unhealthy`).
  - Politica de agregacao: required check falha -> `unhealthy`; optional falha -> `degraded`; checks que lancam excecao tratados como `false` (defensivo).
  - Cliente recebe `serverTimeUtc` + `uptimeSeconds` no payload — base para detectar drift de relogio e troubleshooting.
...
- F2.2 Garantir aplicacao no servidor de: dump, compactacao, checksum, politicas de limpeza e post-script conforme permitido.
- F2.3 Publicar progresso detalhado por etapa para cliente (iniciando, dump, compactacao, verificacao, finalizando). *(`backupStep` implementado)*
- [~] F2.4 Registrar historico e logs no servidor com correlacao (`runId`, `scheduleId`, `clientId`). *(`scheduleId` rastreado; `runId` ja existe internamente em `SchedulerService._executeScheduledBackup` e propagado em `LogContext.setContext(runId, scheduleId)` + `LicensePolicyService.setRunContext`, mas nao formalizado no contrato remoto, nos eventos `backupProgress/Complete/Failed` nem no staging por pasta `remote/<scheduleId>`)*
...
- F2.8 Garantir que execucao remota server-first so inicia por comando explicito recebido do cliente. *(hoje `SchedulerService.start` ainda agenda `_checkTimer = Timer.periodic(Duration(minutes: 1), ...)` que dispara `_checkSchedules` independentemente de comando do cliente)*
- F2.9 Implementar mutex global de execucao de backup no servidor (`maxConcurrentBackups = 1`). *(reusa mutex existente do scheduler + ExecutionQueueService novo, PR-3a 2026-04-19)*
- F2.10 Implementar fila FIFO para execucoes agendadas quando houver backup ativo. *(PR-3a + F2.16 — `ExecutionQueueService` com espelhamento Drift opcional; producao usa persistencia)*
- F2.11 Retornar `409 BACKUP_ALREADY_RUNNING` para disparo manual durante execucao ativa. *(PR-3a — distincao via `queueIfBusy` no `startBackup`)*
- F2.12 Publicar eventos de fila (`backupQueued`, `backupDequeued`, `backupStarted`) com `runId`. *(PR-3a commit 2 — incluem `eventId` UUID + `sequence` monotonico para deduplicacao e reordenacao no cliente)*
- F2.13 Definir e validar maquina de estados da execucao (`queued`, `running`, `completed`, `failed`, `cancelled`) no servidor. *(PR-3a commit 3 — `ExecutionStateMachine` com tabela explicita de transicoes permitidas + `enforceTransition` + `InvalidStateTransitionException`)*
...
- [~] F2.16 Persistir estado de execucao e fila para recuperacao apos reinicio do servidor. *(fila persistida: `execution_queue_items_table` + `DriftExecutionQueuePersistence` + `ExecutionQueueService.initialize()` no bootstrap — 2026-04-19; **estado de execucao em curso** / `RemoteExecutionRegistry` ainda volatil apos restart)*
- [~] F2.17 Adicionar `eventId` e `sequence` em todos eventos de execucao/fila para reprocessamento seguro no cliente. *(PR-3a commit 2: aplicado nos 3 eventos de fila — backupQueued/Dequeued/Started; aplicar em backupProgress/Step/Complete/Failed fica para PR-3 commit final)*
...
- [~] F3.1 Formalizar staging remoto por execucao (`runId`) com metadados de tamanho/hash/chunk. *(staging de arquivo implementado; sem correlacao por `runId`)*
...
- O lock de transferencia deve virar lease com identidade. (**2026-04-23**)
...
- [~] `metrics/health` devem incluir estado operacional do pipeline remoto.
  - `stagingUsageBytes` + `stagingUsageWarn/BlockThresholdBytes` + `stagingUsageLevel` no **health** (2026-04-23; coerente com `metricsResponse`). `warn`/`block` elevam health a `degraded` com mensagem.
...
- Incorporar implementacoes existentes do servidor sem reescrever regra de negocio. *(handlers usam DI: ScheduleRepository/SchedulerService/IScheduleRepository/DatabaseConfigStore — sem reescrever logica)*
- CRUD remoto de configuracao de banco. *(PR-2 commit 5, 2026-04-19; contrato e handler entregues, despacho concreto por tipo via DatabaseConfigStore injetavel)*
- Teste remoto de conexao de banco (`testDatabaseConnection`). *(entregue em 2026-04-19, PR-2 commit 1)*
- Preflight de servidor (`validateServerBackupPrerequisites`).
- [~] Definir contrato de idempotencia para comandos mutaveis:
  - foundation `IdempotencyRegistry` (TTL configuravel, fail-NO-cache em erro, race-safe, opt-in via campo opcional). Entregue em 2026-04-19, PR-2 commit 2.
  - helper `getIdempotencyKey(message)` — extrai chave defensivamente (null para vazio/nao-string).
  - integracao em handlers mutaveis: `startBackup` e `cancelBackup` (PR-2 commit 3, 2026-04-19).
  - integracao em `createSchedule`/`deleteSchedule`/`pauseSchedule`/`resumeSchedule` (PR-2 commit 4, 2026-04-19).
...
- API de execucao/status base:
  - `startBackup` *(PR-2 commit 3)*
  - `cancelBackup` *(PR-2 commit 3)*
  - `getExecutionStatus` *(PR-1)*
  - `getExecutionQueue` (consulta) *(PR-1)*
- `startBackup` deve retornar aceite imediato: *(PR-2 commit 3)*
  - `accepted|queued|running`
  - `runId`
  - `queuePosition` quando aplicavel (campo opcional, `null` em PR-2 ate PR-3b cabear fila)
  - sem aguardar o termino do backup na mesma request (`unawaited` do `_executeBackup` no handler)
- Completar comandos de agendamento faltantes:
  - `createSchedule`, `deleteSchedule`, `pauseSchedule`, `resumeSchedule` *(PR-2 commit 4, 2026-04-19)*
...
- Implementar `cancelQueuedBackup`. *(PR-3a commit 2)*
...
- Entregar diagnostico e suporte operacional: *(PR-3 commit final, contratos prontos; impl real via DI)*
  - `getRunLogs`
  - `getRunErrorDetails`
  - `getArtifactMetadata`
...
- Endurecer lock de transferencia: (**2026-04-23**)
  - trocar lock simples por lease com `owner/runId/acquiredAt/expiresAt` (`.lock` continua indexado por hash; conteudo v1 JSON)
  - limpar locks expirados no bootstrap do servidor (`cleanupExpiredLocks` em `_startSocketServer` antes de `socketServer.start()`)
...
4. Validacao de integridade e cleanup remoto
  ...
  - Rodar cleanup de lease expirado de lock no bootstrap do servidor. (**2026-04-23**, `main.dart`)
...
- `lib/infrastructure/file_transfer_lock_service.dart` + `file_transfer_lease.dart` + `domain/constants/transfer_lease.dart` (**2026-04-23**)
  - Lease com `owner/runId/acquiredAt/expiresAt` (v1 JSON)
  - `cleanupExpiredLocks` no bootstrap do socket (`main.dart`)
...
- Server unit:
  ...
  - validar lease e cleanup de lock expirado (`file_transfer_lock_service_test.dart`, 2026-04-23)
- Client unit:
...
- M1.1 Fechar modelo de execucao do scheduler como **hibrido controlado**. *(ADR aceito em 2026-04-19: [ADR-001](../adr/001-modelo-hibrido-scheduler.md))*
...
- M1.2 Fechar estrategia de transferencia para `v1` como **streaming sem `fileAck`**. *(ADR aceito em 2026-04-19: [ADR-002](../adr/002-transferencia-v1-streaming-sem-fileack.md))*
...
- M1.3 Cravar `protocolVersion` como contrato de compat formal. *(ADR aceito em 2026-04-19: [ADR-003](../adr/003-versionamento-protocolo.md))*
...
- M2.1 Eliminar singletons `_currentClientId/_currentRequestId/_currentScheduleId` em `ScheduleMessageHandler`. *(concluido em 2026-04-19)*
...
- M2.2 Antecipar `executeSchedule` nao-bloqueante para PR-2. *(entregue em 2026-04-19 como `startBackup` separado, mantendo `executeSchedule` legacy para compat v1)*
...
- M2.3 Promover `runId` para o contrato remoto desde PR-2. *(concluido em 2026-04-19)*
...
- M4.1 Tratar `capabilities` como **gate de feature no cliente**, nao so visualizacao. *(infraestrutura completa em 2026-04-19, com adocao no `ServerConnectionProvider`)*
...
- [~] M5.3 Limite e alerta de `stagingUsageBytes` com acao automatica. *(metrica 2026-04-19; thresholds + bloqueio `startBackup` 2026-04-23)*
...
- M5.4 Validacao de payload size por tipo de mensagem. *(implementado em 2026-04-19)*
...
- [~] M6.1 Suite de regressao automatica que roda em todo PR. *(esqueleto entregue em 2026-04-19)*
...
- [~] M7.1 Telemetria minima ja em PR-1, junto com envelope novo. *(parcialmente entregue em 2026-04-19)*
