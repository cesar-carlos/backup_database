#!/usr/bin/env python3
"""Start app in client mode using .env.client."""
...
def main() -> int:
    ensure_project_root()
    divider("Backup Database - Starting CLIENT instance")

    if not command_exists("flutter"):
        cprint("ERRO: comando 'flutter' nao encontrado no PATH.", Color.RED)
        return 1

    if not Path(".env.client").exists():
        cprint("ERRO: .env.client nao encontrado.", Color.RED)
        return 1
    ...
    cprint("Configuracao do cliente carregada (.env.client)", Color.CYAN)
    print()
    ...
    if "DEBUG_APP_MODE=client" not in env_content:
        cprint("FALHA: DEBUG_APP_MODE deve ser 'client'.", Color.RED)
        restore_env()
        return 1
    cprint("OK: modo CLIENT configurado.", Color.GREEN)
    print()

    divider("Iniciando cliente")
    cprint("Use a UI para conectar em localhost:9527", Color.CYAN)
